...
def plotDataSetsWithUpperAndLowerErrorBars(x0, y0, label0, y0_error_lower=np.array(None), y0_error_upper=np.array(None), x1=np.array(None), y1=np.array(None), y1_error_lower=np.array(None), y1_error_upper=np.array(None), label1=None, x2=np.array(None), y2=np.array(None), y2_error_lower=np.array(None), y2_error_upper=np.array(None), label2=None, x3=np.array(None), y3=np.array(None), y3_error_lower=np.array(None), y3_error_upper=np.array(None), label3=None, x4=np.array(None), y4=np.array(None), y4_error_lower=np.array(None), y4_error_upper=np.array(None), label4=None, title=None, xlbl=None, ylbl=None, saveFileName=None, plotLegend=True):
    ...
    if(y1.all() != None):
        if (y1_error_lower.all() == None):
            ...
            plt.errorbar(x1, y1, yerr=(y1_error_lower, y1_error_upper), fmt='o', label = label1)

    if(y2.all() != None):
        if (y2_error_lower.all() == None):
            plt.plot(x2, y2, '^', label=label2)
        else:
            plt.errorbar(x2, y2, yerr=(y2_error_lower, y2_error_upper), fmt='^', label = label2)

    if(y3.all() != None):
        if (y3_error_lower.all() == None):
            plt.plot(x3, y3, 's', label=label3)     #Removed 's-' so does not show data marker on this plot.
        else:
            plt.errorbar(x3, y3, yerr=(y3_error_lower, y3_error_upper), fmt='s', label = label3)

    if(y4.all() != None):
        if (y4_error_lower.all() == None):
            plt.plot(x4, y4, 'd', label=label4)
        else:
            plt.errorbar(x4, y4, yerr=(y4_error_lower, y4_error_upper), fmt='d', label = label4)
...
def plotDataSetsWithUpperAndLowerErrorBarsOnXAxis(x0, y0, label0, x0_error_lower=np.array(None), x0_error_upper=np.array(None), x1=np.array(None), y1=np.array(None), x1_error_lower=np.array(None), x1_error_upper=np.array(None), label1=None, x2=np.array(None), y2=np.array(None), x2_error_lower=np.array(None), x2_error_upper=np.array(None), label2=None, x3=np.array(None), y3=np.array(None), x3_error_lower=np.array(None), x3_error_upper=np.array(None), label3=None, x4=np.array(None), y4=np.array(None), x4_error_lower=np.array(None), x4_error_upper=np.array(None), label4=None, title=None, xlbl=None, ylbl=None, saveFileName=None, plotLegend=True):

    plt.figure(figsize=(10, 6))
    plt.rc('font', family='serif', size=15);

    if (x0_error_lower.all() == None):
        plt.plot(x0, y0, 'x', label=label0)
    else:
        plt.errorbar(x0, y0, xerr=(x0_error_lower, x0_error_upper), fmt='x', label = label0)

    if(y1.all() != None):
        if (x1_error_lower.all() == None):
            plt.plot(x1, y1, 'o', label=label1)
        else:
            plt.errorbar(x1, y1, xerr=(x1_error_lower, x1_error_upper), fmt='o', label = label1)

    if(y2.all() != None):
        if (x2_error_lower.all() == None):
            plt.plot(x2, y2, '^', label=label2)
        else:
            plt.errorbar(x2, y2, xerr=(x2_error_lower, x2_error_upper), fmt='^', label = label2)

    if(y3.all() != None):
        if (x3_error_lower.all() == None):
            plt.plot(x3, y3, 's', label=label3)     #Removed 's-' so does not show data marker on this plot.
        else:
            plt.errorbar(x3, y3, xerr=(x3_error_lower, x3_error_upper), fmt='s', label = label3)

    if(y4.all() != None):
        if (x4_error_lower.all() == None):
            plt.plot(x4, y4, 'd', label=label4)
        else:
            plt.errorbar(x4, y4, xerr=(x4_error_lower, x4_error_upper), fmt='d', label = label4)
    ...
    #plt.ylim(-1, 2);
    if(saveFileName != None): plt.savefig(saveFileName);
    plt.yticks(np.linspace(0,10,0.5), np.array(['0' for i in range(0,1)]));
...
fileDir = '../../../../../../../../Volumes/CBOGGONUSB/Data/20180227-Surface/lysisTrackingOutput/OriglysisEvents.dat';
...
plotDataSetsWithUpperAndLowerErrorBarsOnXAxis(timeStoppedArray, x_onGraph, None, timeStoppedArray_Lerror, timeStoppedArray_Uerror, ylbl='Lysis Event', xlbl='Stopping Time (seconds)', saveFileName='../../../../../../../../Volumes/CBOGGONUSB/Data/20180227-Surface/lysisTrackingOutput/LysisStopTimes');
...
plt.figure(figsize=(10, 6))
plt.rc('font', family='serif', size=15);
plt.errorbar(timeStoppedArray, AvVelocityArray, xerr=(timeStoppedArray_Lerror, timeStoppedArray_Uerror), yerr=AvVelocityErrorArray, fmt='^')
