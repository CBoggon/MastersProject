...
calculatedVelocities = np.asarray(calculatedVelocities);
calculatedVelocityError = np.asarray(calculatedVelocityError);
timeArray = np.asarray(timeArray);

A.plotDataSetsWithErrorBars(timeArray, calculatedVelocities, 'average Velocity', y0_error=calculatedVelocityError, title='Tracked Average Velocities', xlbl='Time (Seconds)', ylbl='Average Velocity (micrometers/second)');
